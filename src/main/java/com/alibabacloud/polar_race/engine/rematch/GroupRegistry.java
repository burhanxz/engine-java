...
public class GroupRegistry {
	private static final int MAX_CACHES = 4;
	...
	public GroupRegistry(File databaseDir) throws Exception {
		...
		for (int cacheIndex = 0; cacheIndex < MAX_CACHES; cacheIndex++) {
			caches[cacheIndex] = ByteBuffer.allocateDirect(CACHE_SIZE);
...
//			}
//			System.exit(-1);
//			System.out.println("GroupRegistry 超时5min");
//		});
	}
			...
			for (;;) {
				...
				try {
					...
					for (int i = lower; i < upper; i++) {
						...
						// 取buffer副本
						ByteBuffer buffer = caches[bufferNum].duplicate();
						buffer.position(0);
		...
		new Thread(() -> {
			...
			try {
				...
				for (int i = Util.GROUPS - 1; i != -1; i = nextGroup(i)) {
					...
					// 跳过空文件
					if ((fileChannel = channels[i]) == null) {
						flag = !flag;
						continue;
					}
					...
					remaining -= buffers;
					// 容器申请缓存
					// 第二关，得到container许可，等待visit消费完，防止污染正在visit的数据
					...
					// 实际操作
					// 缓存key值
					final int nodes = (int) (fileChannel.size() / Util.SIZE_OF_VALUE);
					...
					System.out.println("读key耗时: " + (System.currentTimeMillis() - s));
					...
					// System.out.println("nodes = " + nodes + ", sortedKeyIndex = " +
					// sortedKeyIndex);
					Arrays.sort(container.sortedKeys, 0, sortedKeyIndex);
					System.out.println("排序耗时: " + (System.currentTimeMillis() - s));
